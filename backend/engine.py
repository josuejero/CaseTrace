"""CaseTrace backend data engine and helper utilities."""
...
class CaseDataEngine:
    ...
    def overview(self) -> OverviewResponse:
        ...
        file_summary = gather_file_summary(manifest.get("files", [])) if manifest else {
            "file_count": 0,
            "total_size_bytes": 0,
        }
        with self._connect() as connection:
    ...
    def report_summary(self) -> ReportSummary:
        ...
        if not report or not report.get("path"):
            raise FileNotFoundError("No report found in manifest")
